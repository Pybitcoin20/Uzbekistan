      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
          <div>
            ...
            <h2 className="text-4xl md:text-5xl font-serif font-bold">Interactive <span className="text-samarkand italic">Heritage</span> Map</h2>
          </div>
          
          <div className="flex items-center gap-4">
            <div className="glass p-1 rounded-2xl flex">
              {['Map', 'Satellite', 'Terrain'].map(mode => (
                <button key={mode} className={`px-6 py-2 rounded-xl text-xs font-bold transition-all ${mode === 'Map' ? 'bg-samarkand text-white shadow-lg' : 'text-gray-500 hover:bg-gray-100'}`}>
                  {mode}
                </button>
              ))}
            </div>
        ...
        <div className="relative h-[600px] rounded-[2.5rem] overflow-hidden shadow-2xl shadow-black/10 border-8 border-white">
          {/* Map Placeholder */}
          <div className="absolute inset-0 bg-blue-50 flex items-center justify-center">
            ...
            <div className="relative z-10 text-center px-6">
              ...
              <h3 className="text-2xl font-serif font-bold mb-2">Interactive Map Engine</h3>
            ...
            {/* Mock Markers */}
            <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 2, repeat: Infinity }} className="absolute top-1/4 left-1/3 p-3 bg-white rounded-2xl shadow-xl flex items-center gap-3 border border-samarkand/20">
              ...
              <div>
                ...
                <div className="text-sm font-bold">Registan Square</div>
              </div>
            </motion.div>

            <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} className="absolute bottom-1/3 right-1/4 p-3 bg-white rounded-2xl shadow-xl flex items-center gap-3 border border-silk/20">
              ...
              <div>
                ...
                <div className="text-sm font-bold">Plov Center</div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 glass px-8 py-4 rounded-3xl flex items-center gap-8 shadow-2xl border-white/40">
            {['All', 'Monuments', 'Food', 'Hotels', 'Bazaars'].map(cat => (
              <button key={cat} className="text-xs font-bold uppercase tracking-widest text-gray-500 hover:text-samarkand transition-colors">
                {cat}
              </button>
            ))}
          </div>
